...
class FirewallaAPI:
    ...
    async def create_list(self, name: str, entries: list[str], list_type: str = "domain") -> Optional[str]:
        logger.info(f"Creating Firewalla list '{name}' with {len(entries)} entries (type={list_type})")
        payload = {"name": name, "type": list_type, "targets": entries}
    ...
    async def update_list(self, list_id: str, name: str, entries: list[str], list_type: str = "domain") -> bool:
        """
        ...
        payload = {"name": name, "targets": entries}
